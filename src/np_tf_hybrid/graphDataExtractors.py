...
def neuralGraphTensor(points,selectorMatrix,averageIndex=None):
	"""
	Implementation of algorithm 2. Gets the full set of neural graph weights from a set of points. 
	"""
	selectorMatrix = parameters.get("selectorMatrix")
	averageIndex = parameters.get("averageIndex")

	# Get the difference tensor. This is k x k x n and the i,j vector is the absolute value of the difference 
	if type(selectorMatrix) is np.ndarray:
		selectorMatrix = tf.Constant(selectorMatrix)
	...
	selectionWeights = tf.tensordot(tf.abs(selectorMatrix),diff,[[0],[2]])
	#returns the concatenation.
	return tf.concat([unitWeights,selectionWeights],0)


def l2DistMatrix(points,matrixType=None,sigma=None):
	"""
